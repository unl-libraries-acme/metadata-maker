function buildRedirectURL(servername) {
	...
	var new_url = servername.replace(servername.slice(domain_start,domain_start + servername.slice(domain_start).indexOf('/')),"libpub.unl.edu");
	console.log(new_url)
	return new_url
}

var servername = window.location.href ;
if (servername.indexOf('libpub.unl.edu') < 0) {
